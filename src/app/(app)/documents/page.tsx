          ...
          <Card key={doc.id} id={doc.id}>
            ...
            <CardContent className="space-y-2">
              {doc.versions.map((v) => (
                <div
                  key={v.id}
                  className="flex flex-wrap items-center justify-between gap-2 rounded-lg border p-2.5 text-sm transition-colors hover:bg-secondary/40"
                >
                  <div>
                    <a
                      href={v.fileUrl}
        ...
        {documents.length === 0 && (
          <div className="rounded-xl border bg-card shadow-elevation-sm">
            <EmptyState
              ...
              description="Upload an SOP, CV, or writing sample to start versioning it across applications."
            />
          </div>
        )}
      </div>
